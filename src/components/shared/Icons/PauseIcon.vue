<template>
	<svg width="14" height="14" viewBox="0 0 14 14" fill="#fff" xmlns="http://www.w3.org/2000/svg">
		<g clip-path="url(#clip0_3_214552)">
			<path
				d="M4 0.5H1.5C0.947715 0.5 0.5 0.947715 0.5 1.5V12.5C0.5 13.0523 0.947715 13.5 1.5 13.5H4C4.55228 13.5 5 13.0523 5 12.5V1.5C5 0.947715 4.55228 0.5 4 0.5Z"
				stroke="#fff"
				stroke-linecap="round"
				stroke-linejoin="round"
			/>
			<path
				d="M12.5 0.5H10C9.44772 0.5 9 0.947715 9 1.5V12.5C9 13.0523 9.44772 13.5 10 13.5H12.5C13.0523 13.5 13.5 13.0523 13.5 12.5V1.5C13.5 0.947715 13.0523 0.5 12.5 0.5Z"
				stroke="#fff"
				stroke-linecap="round"
				stroke-linejoin="round"
			/>
		</g>
		<defs>
			<clipPath id="clip0_3_214552">
				<rect width="14" height="14" fill="white" />
			</clipPath>
		</defs>
	</svg>
</template>
